...
def parseargs():    # handle user arguments
	parser = argparse.ArgumentParser(description="Convert clark format profile to CAMI/OPAL format profile.")
	parser.add_argument('--clark', required=True, help='Input clark file name.')
...
def read_clark_file(clark_file):
	# 100225,100225,3020,0.00302616,0.0186459
	taxid_to_abundance = {}
	with(open(clark_file, 'r')) as infile:
		infile.readline()  # skip header
		for line in infile:
			splits = line.strip().split(',')
			taxid = splits[1]
			rank = 'species'
			abundance = '%.5f' % (float(splits[-2]))
...
if __name__ == '__main__':
	...
	taxid_to_abundance = read_clark_file(args.clark)
	del taxid_to_abundance['UNKNOWN']
